...
class Ai:
    ...
    def search(self):  # 要搜索的引擎
        ...
        baidu = "https://zhidao.baidu.com/search?lm=0&rn=10&pn={0}&fr=search&ie=gbk&word=" + urllib.parse.quote(self.issue, encoding='gbk')
        sousou = "http://wenwen.sogou.com/s/?w=" + urllib.parse.quote(self.issue) + "&pg={0}&ch=ww.header.ssda"
        iask =  "https://iask.sina.com.cn/search?searchWord=" + urllib.parse.quote(self.issue) + "&record=1&page={0}"
        so360 = "https://wenda.so.com/search/?q=" + urllib.parse.quote(self.issue)+"&pn={0}"

        seaLis=[baidu,sousou,iask,so360]
        for i in seaLis:
            for j in range(1,4):
                self.threhtml(i.format(j))

        while 1:
            if (self.count == len(seaLis)*3):
                break
